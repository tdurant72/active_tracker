  ...
  <div class="max-w-screen-md mx-auto px-4 py-10">
    ...
    <div class="p-8 flex items-start bg-light-grey rounded-md shadow-lg">
      <form @submit.prevent="createWorkout" class="flex flex-col gap-y-5 w-full">
        ...
        <div v-if="workoutType==='strength'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises" :key="index">
            <div class="flex flex-col md:w-1/3">
              ...
              <input type="text" class="p-2 w-full text-gray-500 focus:outline-none" v-model="item.exercise">
            </div>
            ...
            <img @click="deleteExercise(item.id)" src="@/assets/images/trash-light-green.png" alt="trash" class="h-4 w-auto absolute -left-5 cursor-pointer">
        ...
        <!-- Cardo Training Inputs -->
        <div v-if="workoutType==='cardio'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises" :key="index">
            <div class="flex flex-col md:w-1/3">
              <label for="cardio-type" class="mb-1 text-sm text-at-light-green">Type</label>
              <select name="cardio-type" id="cardio-type" class="p-2 w-full text-gray-500 focus:outline-none" v-model="item.cardioType">
            ...
            <div class="flex flex-col flex-1">
              <label for="pace" class="mb-1 text-sm text-at-light-green">pace (mph)</label>
...
export default {
	...
	setup() {
		...
		const deleteExercise = id => {
			...
			errorMsg.value = 'Error: cannot remove, need at least one exercise';
			setTimeout(() => {
